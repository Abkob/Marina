...
import { CalendarDays, Check, ChevronDown, ChevronLeft, ChevronRight, Clock, List, Minimize2, Maximize2, Plus, RefreshCw, Share, X } from 'lucide-react';
...
export function MobileSchedule(props: Props) {
  ...
  const itemsByDate = useMemo(() => new Map(days.map(day => [day, mobileScheduleItems({ date: day, events: props.events,
    meetings: props.meetings, tasks: props.tasks, day: props.previewByDate.get(day), assignment: props.assignmentsByDate.get(day), blockedTaskIds: props.blockedTaskIds })])),
  [days, props.events, props.meetings, props.tasks, props.previewByDate, props.assignmentsByDate, props.blockedTaskIds]);
  ...
  return <section className={`mobile-schedule mx-auto max-w-xl px-4 pb-4 ${compact ? 'mobile-schedule-compact' : ''}`} aria-label="Mobile schedule">
    <header className="sticky top-0 z-20 -mx-4 bg-white/95 px-4 pb-3 pt-3 backdrop-blur-xl">
      {!compact && <div className="flex items-center justify-between gap-2">
        <div><p className="text-[10px] font-bold uppercase tracking-[0.24em] text-indigo-500">Marina</p><h1 className="font-headline text-[28px] font-bold tracking-tight text-slate-950">Your schedule</h1></div>
        <div className="flex gap-1">
          <button onClick={() => void props.onRefresh()} disabled={props.refreshing || !online} aria-label="Refresh schedule" className="mobile-icon-button text-slate-500 disabled:opacity-40"><RefreshCw size={19} className={props.refreshing ? 'animate-spin' : ''} /></button>
          {!standalone && <button onClick={() => setInstallOpen(true)} aria-label="Install on iPhone" className="mobile-icon-button text-slate-500"><Share size={20} /></button>}
        </div>
      </div>}
      <div className={`${compact ? '' : 'mt-3'} flex items-center justify-between gap-2`}>
        <button onClick={() => { setMonth(date.slice(0, 7) + '-01'); setMonthOpen(!monthOpen); }} aria-expanded={monthOpen} aria-label="Choose date" className="flex min-h-11 min-w-0 flex-1 items-center gap-1 text-sm font-semibold text-slate-800">
          {parseLocalDate(date).toLocaleDateString('en-US', compact ? { weekday: 'short', month: 'short', day: 'numeric' } : { month: 'long', year: 'numeric' })}<ChevronDown size={16} className={`shrink-0 ${monthOpen ? 'rotate-180' : ''}`} />
        </button>
        <button onClick={() => changeDate(today)} className="min-h-11 rounded-xl border border-slate-200 px-4 text-sm font-semibold text-slate-600">Today</button>
        <button onClick={() => { setCompact(!compact); setMonthOpen(false); }} aria-label={compact ? 'Expand calendar controls' : 'Minimize calendar controls'} aria-pressed={compact} title={compact ? 'Expand calendar controls' : 'Compact calendar — preference saved'} className="mobile-size-toggle shrink-0 text-indigo-600">{compact ? <Maximize2 size={16} /> : <Minimize2 size={16} />}<span>{compact ? 'Expand' : 'Compact'}</span></button>
      </div>
      {monthOpen ? <div ref={monthSwipe} aria-label="Month date picker" className="mobile-gesture-surface mt-2 rounded-2xl border border-slate-100 bg-slate-50 p-2">
        ...
        {!compact && <div ref={weekSwipe} className="mobile-gesture-surface mt-2 grid grid-cols-7 gap-1" aria-label="Week dates">
          {days.map(day => <button key={day} aria-label={dateLabel(day)} aria-pressed={day === date} aria-current={day === today ? 'date' : undefined} onClick={() => changeDate(day)} className={`flex min-h-[72px] flex-col items-center justify-center rounded-2xl ${day === date ? 'bg-indigo-600 text-white shadow-md shadow-indigo-200' : day === today ? 'bg-indigo-50 text-indigo-600' : 'text-slate-500'}`}>
            <span className="text-[11px]">{parseLocalDate(day).toLocaleDateString('en-US', { weekday: 'short' })}</span>
            <span className="mt-1 text-lg font-semibold">{parseLocalDate(day).getDate()}</span>
            <span className={`mt-1 h-1 w-1 rounded-full ${(itemsByDate.get(day)?.length ?? 0) > 0 ? day === date ? 'bg-white' : 'bg-indigo-400' : 'bg-transparent'}`} />
          </button>)}
        </div>}
        <div className="mt-2 flex items-center justify-between">
          {compact ? <button onClick={() => void props.onRefresh()} disabled={props.refreshing || !online} aria-label="Refresh schedule" className="mobile-icon-button text-slate-500 disabled:opacity-40"><RefreshCw size={19} className={props.refreshing ? 'animate-spin' : ''} /></button> : <button aria-label="Previous week" className="mobile-icon-button text-slate-400" onClick={() => changeDate(addDays(date, -7))}><ChevronLeft size={19} /></button>}
          <div className="flex rounded-xl bg-slate-100 p-1">{(['agenda', 'day'] as const).map(mode => <button key={mode} onClick={() => setView(mode)} aria-pressed={view === mode} className={`flex min-h-10 items-center gap-2 rounded-lg px-5 text-xs font-semibold ${view === mode ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-500'}`}>{mode === 'agenda' ? <List size={15} /> : <Clock size={15} />}{mode === 'agenda' ? 'Agenda' : 'Day'}</button>)}</div>
          <button aria-label="Next week" className="mobile-icon-button text-slate-400" onClick={() => changeDate(addDays(date, 7))}><ChevronRight size={19} /></button>
        </div>
      </>}
    </header>
    ...
    {props.loading ? <div role="status" className="space-y-3 py-4"><p className="text-sm text-slate-500">Loading your schedule…</p>{[1, 2, 3].map(n => <div key={n} className="h-20 animate-pulse rounded-2xl bg-slate-100" />)}</div> : props.error ? null : <>
      <div className="mb-3 mt-2 flex items-center justify-between gap-2"><div><h2 className="text-sm font-bold text-slate-800">{date === today ? 'Today' : parseLocalDate(date).toLocaleDateString('en-US', { weekday: 'long' })}<span className="ml-2 font-normal text-slate-400">{parseLocalDate(date).toLocaleDateString('en-US', { month: 'short', day: 'numeric' })}</span></h2>
        <p className="mt-1 text-xs text-slate-400">{items.length ? `${items.length} item${items.length === 1 ? '' : 's'} in your day` : 'A little room to breathe'}{props.timezone ? ` · ${props.timezone.replaceAll('_', ' ')}` : ''}</p></div>
        <div className="flex"><button aria-label="Previous day" className="mobile-icon-button text-slate-400" onClick={() => changeDate(addDays(date, -1))}><ChevronLeft size={18} /></button><button aria-label="Next day" className="mobile-icon-button text-slate-400" onClick={() => changeDate(addDays(date, 1))}><ChevronRight size={18} /></button></div>
      </div>
      {view === 'day' ? <>
