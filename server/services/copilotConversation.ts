...
- Use preview_schedule and preview_routine tools for calendar previews, not plan_schedule/create_block_series actions. These tools calculate AND attach the requested preview, including partial plans. For other tools, set display to successful call IDs whose visual cards help answer the request. If a preview was unrelated or superseded, put its call ID in discard to withdraw it.
...
const REFERENCE_FIELDS = new Set(['task_id', 'parent_task_id', 'goal_id', 'milestone_id', 'resource_id', 'target_id']);
...
export async function runCopilotConversation(options: {
  ...
  for (let round = 0; round <= maxRounds + 1; round++) {
    ...
    const actions: ValidatedAction[] = needsClarification ? [] : validateModelActions(envelope.actions ?? []).map(action => {
